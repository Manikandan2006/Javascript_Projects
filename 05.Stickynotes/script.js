...
function getappstorage() {
    return JSON.parse(localStorage.getItems("Mani-sticky") || "[]");
}
...
btnAdd.addEventListener("click", ()=> newsticky());

function saveNotes(note){
    localStorage.setItem("joes-app",JSON.stringify(note));
}
*/
const containerElement=document.getElementById("container");
const btnAdd=document.getElementsByClassName("btnadd")[0];
...
function getAppStorage(){
  return JSON.parse(localStorage.getItem("Mani-stick") || "[]");
}
